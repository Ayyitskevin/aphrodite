...
from aphrodite.domain import JobRecord
...
def render_admin_jobs_index(*, jobs: list[JobRecord], spend: XAISpendSummary) -> str:
    ...
    return _page(
        ...
        <header>
          ...
          <nav><a href="/admin/jobs">Jobs</a><a href="/admin/spend.json">Spend JSON</a></nav>
        </header>
...
def render_admin_job_detail(*, job: JobRecord, spend: XAISpendSummary) -> str:
    ...
    return _page(
        ...
        <header>
          ...
          <nav><a href="/admin/jobs">Jobs</a><a href="/v1/jobs/{_u(job.id)}">Job JSON</a></nav>
        </header>
      ...
      <head>
        ...
        <style>
          ...
          * {{ box-sizing: border-box; }}
          ...
          .status {{ display: inline-block; padding: 3px 8px; border: 1px solid var(--line); background: #eef7f5; color: var(--accent); }}
          .status-failed {{ background: #fff1f0; color: var(--bad); }}
          .status-rendering, .status-queued {{ background: #fff7e6; color: var(--warn); }}
